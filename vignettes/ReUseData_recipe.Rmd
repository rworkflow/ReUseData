...
A simple example of converting existing shell script into a `ReUseData`
recipe.

```{r}
shfile <- system.file("extdata", "demo_cp.sh", package = "ReUseData")
readLines(shfile)
rcp_cp <- recipeMake(shscript = shfile,
                     paramID = c("original"),
                     paramType = c("string"),
                     outputID = "filecopy",
                     outputGlob = "cp_*") 
...
```{r, eval=FALSE}
rcp_cp$original <- "GRCh37_to_GRCh38.chain"
rcp_cp$copy <- "cp_GRCh37_to_GRCh38.chain"
res <- getData(rcp_cp,
        outdir = outdir,
        notes = c("GRCh37", "GRCh38", "human", "liftover", "ensembl", "copy"))
```
