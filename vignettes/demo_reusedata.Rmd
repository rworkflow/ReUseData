...
- create example codes for recipeMake, recipeSearch, recipeLoad, getdata, dataUpdate(), dataSearch(), data retrieval toList(). 
- how to extract the file path column "fpath" from `dataHub` returned by dataSearch()??? toList().  -- done!
- add function: dataUpdate() clean up invalid ReUseData files if $output not exist. (yml,cwl,md5,sh) -- done! Any other situation?  
- `recipeSearch()` `dataSearch()`, if keywords is empty, then return all available recipes/data. -- done!
- If data not successfully generated in `getData()`, should remove the empty .yml file to avoid error in future `dataSearch()` -- done!
...
## Example 5: google bucket

```{r}
## Example:
recipeUpdate(force=TRUE)
recipeSearch(c("gcp", "omni"))
...
getData(rcp,
        ...
        showLog = TRUE)
dataUpdate(dir="/home/qian/workspace/SharedData", outMeta = TRUE)
res <- dataSearch(c("1000g", "omni2.5"))
mcols(res)$fpath  
```
```{r}
recipeUpdate(force=TRUE)
...
getData(rcp, 
        outdir = '/home/qian/workspace/SharedData/gcp_public_data_broad_references_hg19_v0',
        prefix = 'gcp_broad_hg19_v0_1000G_omni2.5',
        notes = c('gcp', 'broad', 'reference', 'hg19', 'v0', '1000G', 'omni2.5', 'b37'),
        showLog = TRUE)
dataUpdate(dir="/home/qian/workspace/SharedData", outMeta = TRUE)
res <- dataSearch(c("1000g", "omni2.5"))
mcols(res)$fpath  
